...
namespace test
{
    public partial class ScoreDataDisplay : Form
    {
        ...
        private void ScoreDataDisplay_Load(object sender, EventArgs e)
        {
        ...
        private void choosecourse_Click(object sender, EventArgs e)
        {
            ...
            {
                SqlDataAdapter sda = new SqlDataAdapter(@"SELECT * FROM [master].[dbo].[ChooseCourse] WHERE Score IS NULL", connection);
            ...
            {
                SqlDataAdapter sda = new SqlDataAdapter(@"SELECT * FROM [master].[dbo].[ChooseCourse] WHERE Score='" + score.Text + "'", connection);
                DataSet ds = new DataSet();
                sda.Fill(ds);
                dataGridView1.DataSource = ds.Tables[0].DefaultView;
            }
